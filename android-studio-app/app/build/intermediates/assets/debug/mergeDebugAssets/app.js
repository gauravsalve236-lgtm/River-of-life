...
function setupMeetingRoomControls() {
  ...
  if (micBtn) {
    micBtn.addEventListener("click", () => {
      ...
      micBtn.classList.toggle("muted", activeMeetingSession.isMuted);
  ...
  // Chat Panel toggle
  const chatBtn = document.getElementById("btn-meet-chat");
  if (chatBtn) chatBtn.addEventListener("click", () => toggleMeetingSidebar("chat"));

  // Hand raise toggle
  const handBtn = document.getElementById("btn-meet-hand");
  if (handBtn) {
    handBtn.addEventListener("click", () => {
      const isRaised = handBtn.classList.toggle("active");
  ...
  const reactBtn = document.getElementById("btn-meet-reactions");
  if (reactBtn) {
    reactBtn.addEventListener("click", (e) => {
      ...
      if (panel) {
        ...
        if (toolbar) panel.style.bottom = `${toolbar.offsetHeight + 10}px`;
      }
    });
  }
  ...
  const prayerBtn = document.getElementById("btn-meet-prayer");
  if (prayerBtn) prayerBtn.addEventListener("click", () => openModal("drawer-meet-prayer-request"));
  
  const closePrayerBtn = document.getElementById("btn-close-meet-prayer-request");
  if (closePrayerBtn) closePrayerBtn.addEventListener("click", () => closeModal("drawer-meet-prayer-request"));

  const prayerForm = document.getElementById("meet-prayer-form");
  if (prayerForm) {
    prayerForm.addEventListener("submit", (e) => {
      e.preventDefault();
  ...
  const membersBtn = document.getElementById("btn-meet-members");
  if (membersBtn) membersBtn.addEventListener("click", () => toggleMeetingSidebar("participants"));

  // Bible Mode Panel toggle
  const bibleBtn = document.getElementById("btn-meet-bible");
  if (bibleBtn) bibleBtn.addEventListener("click", () => toggleMeetingSidebar("bible"));

  // Worship Mode Panel toggle
  const worshipBtn = document.getElementById("btn-meet-worship");
  if (worshipBtn) worshipBtn.addEventListener("click", () => toggleMeetingSidebar("worship"));

  // Host Settings Panel toggle
  const modBtn = document.getElementById("btn-meet-moderator");
  if (modBtn) modBtn.addEventListener("click", () => toggleMeetingSidebar("host"));
  ...
  const closeSidebarBtn = document.getElementById("btn-close-meeting-sidebar");
  if (closeSidebarBtn) {
  ...
  // Leave Call
  const leaveBtn = document.getElementById("btn-meet-leave");
  if (leaveBtn) {
    leaveBtn.onclick = () => {
  ...
  const hostMuteBtn = document.getElementById("btn-host-mute-all");
  if (hostMuteBtn) {
    hostMuteBtn.onclick = () => {
  ...
  const lockBtn = document.getElementById("btn-host-lock-meeting");
  if (lockBtn) {
    lockBtn.onclick = () => {
      isMeetingLocked = !isMeetingLocked;
      const btnText = lockBtn.querySelector("span");
      if (btnText) btnText.textContent = isMeetingLocked ? "Unlock Meeting" : "Lock Meeting";
  ...
  if (recordBtn) {
    recordBtn.onclick = () => {
      ...
      if (isMeetingRecording) {
        ...
        if (overlayTag) overlayTag.style.display = "inline-block";
        ...
        if (overlayTag) overlayTag.style.display = "none";
  ...
  const endBtn = document.getElementById("btn-host-end-meeting");
  if (endBtn) {
    endBtn.onclick = () => {
  ...
  const syncBibleBtn = document.getElementById("btn-sync-bible-verse");
  if (syncBibleBtn) {
    syncBibleBtn.onclick = () => {
      const bookVal = document.getElementById("meeting-bible-book") ? document.getElementById("meeting-bible-book").value : "";
      const chapVal = document.getElementById("meeting-bible-chapter") ? document.getElementById("meeting-bible-chapter").value : "";
      const verseVal = document.getElementById("meeting-bible-verse") ? document.getElementById("meeting-bible-verse").value : "";
      const transVal = document.getElementById("meeting-bible-trans") ? document.getElementById("meeting-bible-trans").value : "";
  ...
  document.querySelectorAll(".quick-song-btn").forEach(btn => {
    btn.onclick = () => {
      const url = btn.dataset.url;
      const urlInput = document.getElementById("worship-youtube-url");
      if (urlInput) urlInput.value = url;
      const modeEl = document.getElementById("worship-youtube-mode");
      const mode = (modeEl && modeEl.value) ? modeEl.value : "audio";
      if (activeMeetingSession) {
        broadcastMeetingEvent(activeMeetingSession.meetingId, {
          type: "PLAY_YOUTUBE",
          url: url,
  ...
  const syncWorshipBtn = document.getElementById("btn-sync-worship-video");
  if (syncWorshipBtn) {
    syncWorshipBtn.onclick = () => {
      const urlEl = document.getElementById("worship-youtube-url");
...
window.nativeLiveKitMeetingManager = {
  ...
  async joinNativeMeeting(roomName, participantName, token = null) {
    ...
    // Add self to participants directory
    this.participants = {};
    const selfUser = participantName || ((state && state.currentUser) ? state.currentUser.username : "Member");
    this.participants[selfUser] = {
      identity: selfUser,
      isLocal: true,
    ...
    try {
      ...
      if (typeof LiveKit !== "undefined" && LiveKit.Room) {
        ...
        // Connect if server URL or token provided
        if (token) {
          try {
            const lkUrl = window.LIVEKIT_SERVER_URL || "wss://river-of-life.livekit.cloud";
            await this.room.connect(lkUrl, token);
            await this.room.localParticipant.enableCameraAndMicrophone();
          } catch(lkErr) {
            console.warn("[Native LiveKit] Server connection fallback to hardware local stream:", lkErr);
          }
        }
      }

      // Render local participant tile instantly in #river-video-grid
      this.renderLocalParticipantTile(selfUser);

      // Acquire local mic & camera
      await this.publishLocalHardwareTracks();

      // Refresh Participant Roster UI
      this.updateParticipantRosterUI();
  ...
  renderLocalParticipantTile(name) {
    ...
    localTile.innerHTML = `
      ...
      <div class="river-participant-badge">
        <span id="badge-mic-icon">${this.isMuted ? "🔇" : "🎙️"}</span>
        <span>${name} (You)</span>
      </div>
    `;
    if (this.localStream) {
      const videoEl = localTile.querySelector("#local-video-element");
      if (videoEl) videoEl.srcObject = this.localStream;
    }
  },

  // Render Remote Participant Tile
  renderRemoteParticipantTile(participant) {
    const grid = document.getElementById("river-video-grid");
    if (!grid) return;

    const tileId = `tile-remote-${participant.identity}`;
    let remoteTile = document.getElementById(tileId);
    if (!remoteTile) {
    ...
    const initial = participant.identity ? participant.identity.charAt(0).toUpperCase() : "R";

    remoteTile.innerHTML = `
      <video id="remote-video-${participant.identity}" class="river-video-element" autoplay playsinline></video>
      <audio id="remote-audio-${participant.identity}" autoplay></audio>
      <div id="remote-avatar-${participant.identity}" class="river-avatar-placeholder" style="display: flex;">
        <div class="river-avatar-circle">${initial}</div>
        <div style="font-size: 13px; font-weight: 700; color: #cbd5e1;">${participant.identity}</div>
      </div>
      <div class="river-participant-badge">
        <span id="remote-mic-${participant.identity}">🎙️</span>
  ...
  removeRemoteParticipantTile(identity) {
    const tile = document.getElementById(`tile-remote-${identity}`);
  ...
  toggleMic() {
    ...
    const loggedIn = (state && state.currentUser) ? state.currentUser.username : "Member";
    if (this.participants[loggedIn]) this.participants[loggedIn].isMuted = this.isMuted;
    this.updateParticipantRosterUI();
  ...
  toggleCam() {
    ...
    if (this.room && this.room.localParticipant) {
      this.room.localParticipant.setCameraEnabled(!this.isCamOff);
    }
    const videoEl = document.getElementById("local-video-element");
    ...
    const loggedIn = (state && state.currentUser) ? state.currentUser.username : "Member";
    if (this.participants[loggedIn]) this.participants[loggedIn].isCamOff = this.isCamOff;
    this.updateParticipantRosterUI();
  ...
  bindRoomEvents() {
    ...
    this.room.on(RoomEvent.ParticipantConnected, (participant) => {
      ...
      this.participants[participant.identity] = { identity: participant.identity, isLocal: false };
      this.renderRemoteParticipantTile(participant);
    ...
    this.room.on(RoomEvent.ParticipantDisconnected, (participant) => {
      ...
      this.removeRemoteParticipantTile(participant.identity);
      showToast(`${participant.identity} left the room`);
    });

    this.room.on(RoomEvent.TrackSubscribed, (track, publication, participant) => {
      console.log("[Native LiveKit] Track Subscribed:", track.kind, participant.identity);
      if (track.kind === "video") {
        const videoEl = document.getElementById(`remote-video-${participant.identity}`);
        const avatarEl = document.getElementById(`remote-avatar-${participant.identity}`);
        if (videoEl) {
        ...
        const audioEl = document.getElementById(`remote-audio-${participant.identity}`);
    ...
    this.room.on(RoomEvent.ActiveSpeakersChanged, (speakers) => {
      document.querySelectorAll(".river-video-tile").forEach(t => t.classList.remove("speaking-active"));
      speakers.forEach(speaker => {
        const tileId = speaker.isLocal ? "tile-local-participant" : `tile-remote-${speaker.identity}`;
        const tile = document.getElementById(tileId);
        if (tile) tile.classList.add("speaking-active");
      });
    });
    ...
    this.room.on(RoomEvent.DataReceived, (payload, participant) => {
      try {
        ...
        } else if (data.type === "HOST_MUTE") {
          const loggedIn = (state && state.currentUser) ? state.currentUser.username : "Member";
          if (data.target === loggedIn || data.target === "ALL") {
            if (!this.isMuted) this.toggleMic();
            showToast("Host muted your microphone 🤐");
          }
        } else if (data.type === "HOST_END") {
          showToast("Host ended the meeting 🛑");
          exitLiveMeetingRoomDirectly();
        }
      } catch(e) {
        // Raw string text chat fallback
        const str = new TextDecoder().decode(payload);
  ...
  addChatMessage(sender, text) {
    ...
    // Show chat badge indicator if drawer is closed
    const chatDot = document.getElementById("teams-chat-unread-dot");
    if (chatDot) chatDot.style.display = "block";
  },

  // Send Data Message across WebRTC Channel
  sendDataMessage(payloadObj) {
    if (this.room && this.room.localParticipant) {
      const dataStr = JSON.stringify(payloadObj);
      this.room.localParticipant.publishData(new TextEncoder().encode(dataStr), { reliable: true });
    }
  },

  // 9. Update Participant Roster UI
  updateParticipantRosterUI() {
    const rosterList = document.getElementById("river-participants-list");
    if (!rosterList) return;

    rosterList.innerHTML = "";
    const isHost = activeMeetingSession && activeMeetingSession.isHost;

    const currentLoggedIn = (state && state.currentUser) ? state.currentUser.username : "Member";

    // Ensure self is in participants list
    if (!this.participants[currentLoggedIn]) {
      this.participants[currentLoggedIn] = {
        identity: currentLoggedIn,
        isLocal: true,
        isMuted: this.isMuted,
        isCamOff: this.isCamOff
      };
    ...
    Object.values(this.participants).forEach(p => {
      const row = document.createElement("div");
      row.style.cssText = "display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; background: rgba(255,255,255,0.05); border-radius: 12px; border: 1px solid rgba(255,255,255,0.08); color: #fff;";
      
      const isMe = p.isLocal || p.identity === currentLoggedIn;
      const initial = p.identity ? p.identity.charAt(0).toUpperCase() : "M";

      row.innerHTML = `
        <div style="display: flex; align-items: center; gap: 10px;">
          <div style="width: 34px; height: 34px; border-radius: 50%; background: linear-gradient(135deg, #6366f1, #a855f7); display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 14px;">
            ${initial}
          </div>
          <div style="display: flex; flex-direction: column; text-align: left;">
            <span style="font-size: 13px; font-weight: 700; color: #f8fafc;">${p.identity} ${isMe ? "(You)" : ""}</span>
            <span style="font-size: 10.5px; color: #94a3b8;">${isMe && isHost ? "👑 Host" : "Participant"}</span>
          </div>
        </div>
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="font-size: 14px;">${(isMe ? this.isMuted : p.isMuted) ? "🔇" : "🎙️"}</span>
          ${isHost && !isMe ? `
            <button onclick="hostMuteSingleParticipant('${p.identity}')" style="background: rgba(239, 68, 68, 0.2); border: 1px solid #ef4444; color: #fff; padding: 4px 8px; border-radius: 6px; font-size: 10px; font-weight: 700; cursor: pointer;">
              Mute
            </button>
          ` : ""}
        </div>
  ...
  // 10. Host Control Functions
  muteAllParticipants() {
    this.sendDataMessage({ type: "HOST_MUTE", target: "ALL" });
    showToast("Host muted all participants 🤐");
  },

  muteSingleParticipant(identity) {
    this.sendDataMessage({ type: "HOST_MUTE", target: identity });
    showToast(`Muted participant ${identity} 🤐`);
  },

  endMeetingForEveryone() {
    this.sendDataMessage({ type: "HOST_END" });
...
window.hostMuteSingleParticipant = (id) => window.nativeLiveKitMeetingManager.muteSingleParticipant(id);
