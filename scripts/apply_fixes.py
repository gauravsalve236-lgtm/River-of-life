...
for path in (Path('app.js'),Path('android-studio-app/app/build/intermediates/assets/debug/mergeDebugAssets/app.js')):
    ...
    # iOS: bypass Safari choice and immediately use the in-app Jitsi flow.
    ios_patch="""\n(function(){\n  const ios=/iPad|iPhone|iPod/.test(navigator.userAgent)||(navigator.platform==='MacIntel'&&navigator.maxTouchPoints>1);\n  if(!ios||window.__rolInAppMeetingPatch)return;window.__rolInAppMeetingPatch=true;\n  const original=window.triggerJoinMeetingFlow;\n  if(typeof original==='function')window.triggerJoinMeetingFlow=function(id){original(id);setTimeout(function(){try{proceedJoinMeetingInApp()}catch(e){}},0)};\n  const hide=function(){const e=document.getElementById('modal-ios-meeting-choice');if(e)e.style.display='none'};hide();new MutationObserver(hide).observe(document.documentElement,{childList:true,subtree:true});\n})();\n"""
    if 'window.__rolInAppMeetingPatch' not in s: s+=ios_patch
