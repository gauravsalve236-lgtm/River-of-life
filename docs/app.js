...
function downloadShareCard() {
  ...
  ctx.fillText(refText, canvas.width / 2, 480);
...
function getJitsiServerDomain() {
  const configured = localStorage.getItem("rol_jitsi_server");
  if (configured && configured.trim()) {
    return configured.replace(/^https?:\/\//, '').replace(/\/+$/, '');
  }
  return "meet.darmstadt.social";
}
...
(function(){
  ...
  const allow = () => document.querySelectorAll('iframe').forEach(f => {
    if (/jitsi|darmstadt|meet/i.test(f.src || '') || (f.id === 'webrtc-room-iframe')) {
